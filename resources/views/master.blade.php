...
<head>
    ...
    if (isset($css) && is_array($css)) {
        foreach ($css as $href) {
            echo "<link rel=\"stylesheet\" href=\"" . url('assets/css/' . $href . '.css') . "?v=" . trim(exec('git log --pretty="%h" -n1 HEAD')) . "\"></script>";
        }
    }
    ?>
...
<!-- Others Theme Scripts -->
<?php
if (isset($js) && is_array($js)) {
    foreach ($js as $src) {
        echo "<script src=\"" . url('assets/js/' . $src . '.js') . "?v=" . trim(exec('git log --pretty="%h" -n1 HEAD')) . "\"></script>";
    }
}
?>
